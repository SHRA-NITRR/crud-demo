...
const mapStateToProps = ({ addUser }) => {
  const userObjectToUpdate = addUser.userList.find(
    ({ userName }) => userName === addUser.userToUpdate
  );
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    addUser: (name, userName, email, role) =>
      dispatch(addUserAction(name, userName, email, role)),
    updateUser: (name, userName, email, role) =>
      dispatch(updateUser(name, userName, email, role))
  };
};
